...
Vert.x-web是Vert.x中一组构建web应用的工具。你可以将它一把瑞士军刀，用于构建现代化的、可扩展的web应用。

Vert.x core提供了一些比较低级的功能用于处理HTTP，这对一些应用而言足够了。

Vert.x-Web基于Vert.x core开发并提供了更多的功能，可以更加轻松地构建用于生产的web应用。

它是Vert.x 2.x中http://pmlopes.github.io/yoke/[Yoke] 的接任者，并且从其他项目中汲取了灵感，包括Node.js的http://expressjs.com/[Express]
和Ruby的http://www.sinatrarb.com/[Sinatra] 。

Vert.x-Web是一个强大的、没有固定范式的、可嵌入的组件。You just use the parts you want and nothing more. Vert.x不是一个容器（像Tomcat那样）。

你可以使用Vert.x-web构建一些典型的后端web应用，比如RESTful web应用，实时web应用（服务端主动发送消息），或者其他你能想到的web应用。Vert.x-web并不关心你用它来构建何种应用。这完全取决于你想构建何种应用，而不是取决于Vert.x-web。
...
Template engines are described by `link:../../apidocs/io/vertx/ext/web/common/template/TemplateEngine.html[TemplateEngine]`. In order to render a template
`link:../../apidocs/io/vertx/ext/web/common/template/TemplateEngine.html#render-io.vertx.core.json.JsonObject-java.lang.String-io.vertx.core.Handler-[render]` is used.
...
`io.vertx:vertx-web-templ-mvel:4.0.0`. Create an instance of the MVEL template engine using:
...
`io.vertx:vertx-web-templ-jade:4.0.0`. Create an instance of the Jade template engine using:
...
`io.vertx:vertx-web-templ-handlebars:4.0.0`. Create an instance of the Handlebars template engine
...
`io.vertx:vertx-web-templ-thymeleaf:4.0.0`. Create an instance of the Thymeleaf template engine
...
`io.vertx:vertx-web-templ-freemarker:4.0.0`. Create an instance of the Apache FreeMarker template engine
...
`io.vertx:vertx-web-templ-pebble:4.0.0`. Create an instance of the Pebble template engine
...
To use Rocker, then add `io.vertx:vertx-web-templ-rocker:4.0.0` as a dependency to your project.
...
`io.vertx:vertx-web-templ-httl:4.0.0`. Create an instance of the HTTL template engine
...
`io.vertx:vertx-web-templ-rythm:4.0.0`. Create an instance of the Rythm template engine
