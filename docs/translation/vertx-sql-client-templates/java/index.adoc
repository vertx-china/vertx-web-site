...
Anemic JSON 行映射是一个简单的模板映射，它用 `link:../../apidocs/io/vertx/sqlclient/Row.html#toJson--[toJson]` 来在数据行和JSON对象之间进行映射。
...
Anemic JSON 参数映射是一个在模板参数和JSON对象之间的简单映射：
...
行映射是通过用每行key/value pair来创建 `JsonObject` 实现的，然后
...
_codegen_ 注解执行器在编译期间生成这些类，这是Java编译器的特性，所以 _不能有额外步骤_ ，这仅仅需要在构建时保证配置的正确性：
