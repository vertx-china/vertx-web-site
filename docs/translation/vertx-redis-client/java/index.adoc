...
该模块允许在 Redis 中保存、检索、搜索和删除数据。Redis 是一个开源的、先进的键值存储数据库。
它通常被称为数据结构服务器，因为键可以包含字符串、散列、列表、集合和排序集合。
...
* 基数统计 HyperLogLog - Commands to approximating the number of distinct elements in a multiset, a HyperLogLog.
* 键 Keys - 使用 key 相关的命令。
...
配置中存在 `password` 或者 `select` 数据库，在连接时命令会自动执行。
...
例如，该模块提供了一个干净的 API 来执行命令，而不需要自己手写命令。
如果想要获取键的值，可以这样做：
...
返回的对象是泛型类型，它允许从基本的 redis 类型转换为您的语言类型。
例如，如果您的 response 类型为`INTEGER`，则可以通过任意数值基元类型 `int`、`long` 获取该值。
...
客户端将在后台监听哨兵的事件。当哨兵通知我们切换了主机时，就会向客户端发送一个异常，您可以决定下一步做什么。
...
在这种情况下，配置需要知道群集的一个或多个成员。
此列表用于向群集请求当前配置，这意味着如果列出的任何成员不可用，则将跳过该成员。

在群集模式下将建立到每个节点的连接。
在执行命令时需要特别小心，建议阅读Redis手册以了解集群如何工作。
在此模式下操作的客户端会尽量识别执行的命令使用哪个槽，以便在正确的节点上执行它。
...
请注意，高可用模式和群集模式报告的服务器地址始终位于 TCP 地址上，而不是域套接字上。
...
* `poolRecycleTimeout` 保持连接池池中打开的连接等待然后关闭的超时时间 (默认 `15_000`)

连接池对于避免自定义连接管理非常有用，例如，您可以只使用：
...
需要注意的是，没有获取或返回任何连接都是由连接池处理的。
但是，当超过1个并发请求尝试从连接池中获取连接时，可能会出现一些可伸缩性问题。
为了克服这个问题，我们需要对连接池进行调优。
常见的配置是将连接池的最大大小设置为可用CPU核心数，并允许请求从连接池中获取连接以进行排队：
...
在本例中，客户端对象将在重新连接时被替换，应用程序将重试最多16次，回退时间最长可达1280ms。
通过丢弃客户端，我们可以确保所有旧的飞行响应都会丢失，而所有新的响应都会在新的连接上。
