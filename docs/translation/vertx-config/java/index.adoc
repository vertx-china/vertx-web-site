...
**Config Retriever** 实例化并被Vert.x应用所使用。它配置了一系列配置中心（Configuration store）
...
默认情况下，Config Retriever被下列配置中心所配置（以此
为顺序）：
...
如下可见更多关于覆盖规则和可用配置中心的细节信息。每一种配置中心都可以标记为
`可选的（optional）` 。如果正在从一个可选的配置中心中获取（或处理）配置的时候，捕获到一个失败事件，那么这个失败事件会
...
配置中心的声明顺序非常重要，因为
它定义了覆盖顺序。对于冲突的key， _后_ 声明的配置中心会
覆盖之前的。我们举个例子。我们
有两个配置中心：
...
一些配置中心要求额外的配置（比如路径……）。这项配置需要用 `link:../../apidocs/io/vertx/config/ConfigStoreOptions.html#setConfig-io.vertx.core.json.JsonObject-[setConfig]` 方法传入一个JSON Object来指定。
...
这个配置中心不支持 `format` 配置项。默认地，获取到的值
...
尽管Vert.x对于开箱即用的配置中心都提供了支持，但是Vert.x Config还是提供了
额外的格式以供您的应用使用。
