...
`vault_config` 对象可以包含 HTTP client / Web client 的配置，例如
trust stores、timeout、certificates、port和host。 `path` 和 `host` 强制必填。 `path` 指定了需要
获取的secret。 `host` 是Vault服务的hostname。默认情况下使用8200端口。
...
您可以使用root token，但是不建议这样做。当token被回收，则对secret的访问也会
受阻。如果token可刷新，则当它过期的时候会被刷新。
...
如果生成的token可刷新，那么在它过期的时候会
自动刷新。 `renew-window` 是更新token有效性的时间窗口。
