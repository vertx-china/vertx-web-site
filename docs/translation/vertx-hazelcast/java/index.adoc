...
如果通过命令行来使用 Vert.x，对应集群管理器的 `jar` 包（名为 `vertx-hazelcast-4.0.0.jar` ）
...
NOTE: 如果你使用了其他版本的 Hazelcast core 依赖，请确保与 Kubernetes discovery 插件的兼容性.
...
spec:
 strategy:
   ...
   rollingParams:
     ...
     maxSurge: 1 <2>
----
<1> 升级过程中，不可用的 Pod 最大数量
<2> 实际创建的 Pod 数量 小于等于（期望的 Pod 数量 + maxSurge）
...
为了尽量减少 Vert.x 集群适应新拓扑的时间，您可以使用外部数据节点，并将 Vert.x 节点标记为 https://docs.hazelcast.org/docs/latest/manual/html-single/#enabling-lite-members[_Lite Members_]。

_Lite Members_ 像普通成员一样加入 Hazelcast 集群，但是他们不拥有任何数据分区。
因此，添加或删除此类成员时，Hazelcast 不需要迁移数据分区。

IMPORTANT: 您必须事先启动外部数据节点，因为 Hazelcast 不会只使用 _Lite Members_ 节点创建集群。

启动外部数据节点可以使用 Hazelcast 分发启动脚本，或以编程方式进行。

可以在XML配置中将Vert.x节点标记为 _Lite Members_ 节点：
